...
def test_recorded_molecular_best_settings_are_not_generic_defaults():
    profiles = yaml.safe_load((ROOT / "configs" / "reproduction_profiles.yaml").read_text(encoding="utf-8"))["profiles"]
    assert profiles["mb2d_reference_v2_seed0"]["status"] == "corrected_exact"
    assert profiles["dw4_reference_v2_seed2"]["am"]["grad_clip"] == 1.0
    assert profiles["lj13_reference_v2_seed2"]["fm"]["batch_size"] == 64
    assert profiles["lj13_reference_v2_seed2"]["am"]["steps"] == 10000
