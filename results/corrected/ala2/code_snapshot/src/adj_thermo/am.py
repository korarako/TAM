...
from adj_thermo.model.painn_ala2_ti import (
    ...
    build_ala2_cpainn_velocity_model,
    is_ala2_cpainn_velocity_params,
...
from adj_thermo.model.painn_ala2_chiro import (
    ...
    build_ala2_chiro_cpainn_velocity_model,
    is_ala2_chiro_cpainn_velocity_params,
...
from adj_thermo.model.painn_amboltz_aligned import apply_painn_amboltz_aligned_velocity_params
...
def _apply_velocity(
    ...
    if model_type == "ala2_cpainn":
        if painn_config is None or painn_model is None or painn_state is None:
            raise ValueError("Ala2 cPaiNN velocity call requires painn_model, painn_state, and painn_config.")
        return apply_ala2_cpainn_velocity_params(painn_model, params, painn_state, x, t, beta, problem, painn_config)
    if model_type == "ala2_chiro_cpainn":
        if painn_config is None or painn_model is None or painn_state is None:
            raise ValueError("Ala2 Chiro cPaiNN velocity call requires painn_model, painn_state, and painn_config.")
        return apply_ala2_chiro_cpainn_velocity_params(painn_model, params, painn_state, x, t, beta, problem, painn_config)
    if model_type == "ala2_chiro_painn_jax":
        if painn_config is None or painn_model is None or painn_state is None:
            raise ValueError("Ala2 Chiro painn-jax velocity call requires painn_model, painn_state, and painn_config.")
        return apply_ala2_chiro_painn_jax_velocity_params(painn_model, params, painn_state, x, t, beta, problem, painn_config)
    if model_type == "painn":
        if painn_config is None:
            raise ValueError("PaiNN velocity call requires painn_config.")
        if str(painn_config.get("painn_backend", "jax")) == "amboltz_aligned":
            return apply_painn_amboltz_aligned_velocity_params(params, x, t, beta, problem, painn_config)
...
def am_loss_fn(
    ...
    term = (2.0 / sigma) * (v_new - v_base) + sigma * flat_a
...
def train_am(
    ...
    painn_config = None
    egnn_config = None
    save_as_ala2_chiro_painn_jax = is_ala2_chiro_painn_jax_velocity_params(base_params) or str(model_type) == "ala2_chiro_painn_jax"
    save_as_ala2_chiro_cpainn = is_ala2_chiro_cpainn_velocity_params(base_params) or str(model_type) == "ala2_chiro_cpainn"
    save_as_ala2_cpainn = is_ala2_cpainn_velocity_params(base_params) or str(model_type) == "ala2_cpainn"
    ...
    if save_as_ala2_chiro_painn_jax:
        if not is_ala2_chiro_painn_jax_velocity_params(base_params):
            raise ValueError("Ala2 Chiro painn-jax AM requires an ala2_chiro_painn_jax FM package with model_type/config/params/state.")
        ...
        base_painn_params = jax.tree_util.tree_map(lambda x: jnp.asarray(x), ala2_chiro_painn_jax_trainable_params_from_package(base_params))
        params = jax.tree_util.tree_map(lambda x: jnp.array(x, copy=True), base_painn_params)
        base_params_train = base_painn_params
    elif save_as_ala2_chiro_cpainn:
        if not is_ala2_chiro_cpainn_velocity_params(base_params):
            raise ValueError("Ala2 Chiro cPaiNN AM requires an ala2_chiro_cpainn FM package with model_type/config/params/state.")
        model_type = "ala2_chiro_cpainn"
        painn_config = base_params["config"]
        painn_model = build_ala2_chiro_cpainn_velocity_model(painn_config)
        painn_state = jax.tree_util.tree_map(lambda x: jnp.asarray(x), base_params["state"])
        base_painn_params = jax.tree_util.tree_map(lambda x: jnp.asarray(x), ala2_chiro_cpainn_trainable_params_from_package(base_params))
        params = jax.tree_util.tree_map(lambda x: jnp.array(x, copy=True), base_painn_params)
        base_params_train = base_painn_params
    elif save_as_ala2_cpainn:
        if not is_ala2_cpainn_velocity_params(base_params):
            raise ValueError("Ala2 cPaiNN AM requires an ala2_cpainn FM package with model_type/config/params/state.")
        model_type = "ala2_cpainn"
        painn_config = base_params["config"]
        painn_model = build_ala2_cpainn_velocity_model(painn_config)
        painn_state = jax.tree_util.tree_map(lambda x: jnp.asarray(x), base_params["state"])
        base_painn_params = jax.tree_util.tree_map(lambda x: jnp.asarray(x), ala2_cpainn_trainable_params_from_package(base_params))
        params = jax.tree_util.tree_map(lambda x: jnp.array(x, copy=True), base_painn_params)
        base_params_train = base_painn_params
    elif save_as_painn:
        ...
        model_type = "painn"
        painn_config = base_params["config"]
        painn_backend = str(base_params.get("painn_backend", painn_config.get("painn_backend", "jax")))
        if painn_backend == "amboltz_aligned":
            painn_model = None
            painn_state = base_params.get("state", None)
            base_painn_params = jax.tree_util.tree_map(lambda x: jnp.asarray(x), base_params["params"])
        else:
    ...
    def save_am_package(path: Path, current_params: Params) -> None:
        if save_as_ala2_chiro_painn_jax:
            ...
            payload = {"model_type": "ala2_chiro_painn_jax", "config": painn_config, "params": model_params, "state": painn_state}
            if extra_params:
                payload["extra_params"] = extra_params
            save_pickle(path, payload)
        elif save_as_ala2_chiro_cpainn:
            model_params, extra_params = split_ala2_chiro_cpainn_trainable_params(current_params)
            payload = {"model_type": "ala2_chiro_cpainn", "config": painn_config, "params": model_params, "state": painn_state}
            if extra_params:
                payload["extra_params"] = extra_params
            save_pickle(path, payload)
        elif save_as_ala2_cpainn:
            model_params, extra_params = split_ala2_cpainn_trainable_params(current_params)
            payload = {"model_type": "ala2_cpainn", "config": painn_config, "params": model_params, "state": painn_state}
            if extra_params:
                payload["extra_params"] = extra_params
            save_pickle(path, payload)
        elif save_as_painn:
            backend = str(painn_config.get("painn_backend", "jax")) if painn_config is not None else "jax"
            if backend == "amboltz_aligned":
                payload = {
                    "model_type": "painn",
                    "painn_backend": "amboltz_aligned",
                    "config": painn_config,
                ...
                payload = {"model_type": "painn", "painn_backend": backend, "config": painn_config, "params": model_params, "state": painn_state}
                if extra_params:
                    payload["extra_params"] = extra_params
            save_pickle(path, payload)
        elif save_as_egnn:
            save_pickle(path, {"model_type": "egnn", "config": egnn_config, "params": current_params})
        else:
            save_pickle(path, current_params)

    print(
        f"[AM] prepared jitted train step K={int(K)} batch={int(batch_size)} "
        f"loss_steps={int(am_num_loss_steps)} keep_last={int(am_keep_last_steps)}; first step includes JIT compile",
        flush=True,
    )
