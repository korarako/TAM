...
from adj_thermo.model.painn_ala2_ti import (
    ala2_cpainn_trainable_params_from_package,
    apply_ala2_cpainn_velocity_params,
    init_ala2_cpainn_velocity,
    split_ala2_cpainn_trainable_params,
)
from adj_thermo.model.painn_ala2_chiro import (
    ala2_chiro_cpainn_trainable_params_from_package,
    apply_ala2_chiro_cpainn_velocity_params,
    init_ala2_chiro_cpainn_velocity,
    split_ala2_chiro_cpainn_trainable_params,
)
...
from adj_thermo.model.painn_amboltz_aligned import (
    apply_painn_amboltz_aligned_velocity_params,
    init_painn_amboltz_aligned_velocity,
)
...
def load_beta_datasets(
    ...
    for beta_i, beta in enumerate(beta_grid):
        ...
        elif data_dir.name == "ala2":
            from adj_thermo.problem.ala2 import load_ala2_positions_for_beta

            arr = load_ala2_positions_for_beta(float(beta))
            print(f"[FM] beta={float(beta):.8f} loaded Ala2 external dataset for {path.name}: shape={arr.shape}", flush=True)
        else:
...
def _fm_loss_from_stacked(
    ...
    if model_type == "ala2_cpainn":
        if painn_config is None or painn_model is None or painn_state is None:
            raise ValueError("Ala2 cPaiNN FM loss requires painn_model, painn_state, and painn_config.")
        pred = apply_ala2_cpainn_velocity_params(painn_model, params, painn_state, x_t, t, beta, problem, painn_config)
    elif model_type == "ala2_chiro_cpainn":
        if painn_config is None or painn_model is None or painn_state is None:
            raise ValueError("Ala2 Chiro cPaiNN FM loss requires painn_model, painn_state, and painn_config.")
        pred = apply_ala2_chiro_cpainn_velocity_params(painn_model, params, painn_state, x_t, t, beta, problem, painn_config)
    elif model_type == "ala2_chiro_painn_jax":
        if painn_config is None or painn_model is None or painn_state is None:
            raise ValueError("Ala2 Chiro painn-jax FM loss requires painn_model, painn_state, and painn_config.")
        pred = apply_ala2_chiro_painn_jax_velocity_params(painn_model, params, painn_state, x_t, t, beta, problem, painn_config)
    elif model_type == "painn":
        if painn_config is None:
            raise ValueError("PaiNN FM loss requires painn_config.")
        if str(painn_backend) == "amboltz_aligned":
            pred = apply_painn_amboltz_aligned_velocity_params(params, x_t, t, beta, problem, painn_config)
        else:
...
def _write_painn_radial_cutoff_summary(run_dir: Path, config: dict[str, Any]) -> None:
    try:
        radius = float(config["radius"])
        n_rbf = int(config["n_rbf"])
    except Exception:
        return
    r = np.linspace(0.0, radius, 512, dtype=np.float64)
    x = np.clip(r / max(radius, 1.0e-12), 0.0, 1.0)
    cosine = np.where(r < radius, 0.5 * (np.cos(np.pi * x) + 1.0), 0.0)
    poly = np.where(r < radius, 1.0 - 10.0 * x**3 + 15.0 * x**4 - 6.0 * x**5, 0.0)
    summary = {
        "adtm_jax_painn": {
            "radial_basis": str(config.get("radial_basis", "painn_jax.gaussian_rbf")),
            "cutoff": str(config.get("cutoff", "painn_jax.cosine_cutoff")),
            "radius": radius,
            "n_rbf": n_rbf,
        },
        "amboltz_reference": {
            ...
            "painn_py_cutoff": "polynomial",
            "painn_aligned_py_cutoff": "cosine",
        },
        "cutoff_curve_comparison": {
            "cosine_vs_polynomial_max_abs_diff": float(np.max(np.abs(cosine - poly))),
            "cosine_vs_polynomial_mean_abs_diff": float(np.mean(np.abs(cosine - poly))),
        },
    }
    try:
        with (run_dir / "painn_radial_cutoff_summary.json").open("w", encoding="utf-8") as f:
    ...
    try:
        ...
        ax.plot(r, cosine, label="ADTM/painn_jax cosine")
        ax.plot(r, poly, label="amboltz polynomial", linestyle="--")
        ax.set_xlabel("distance")
        ax.set_ylabel("cutoff")
        ax.set_title("PaiNN cutoff comparison")
        ax.legend(frameon=False)
        fig.tight_layout()
        fig.savefig(run_dir / "painn_radial_cutoff_comparison.png", dpi=180)
        plt.close(fig)
    except Exception:
        pass
...
def train_fm(
    ...
    egnn_config = None
    if model_type == "ala2_cpainn":
        package, painn_model = init_ala2_cpainn_velocity(
            init_key,
            problem,
        ...
        params = ala2_cpainn_trainable_params_from_package(package)
        painn_state = package["state"]
        painn_config = package["config"]
        _write_painn_radial_cutoff_summary(run_dir, painn_config)
    elif model_type == "ala2_chiro_cpainn":
        package, painn_model = init_ala2_chiro_cpainn_velocity(
            init_key,
            problem,
            hidden_dim=hidden_dim,
            n_layers=n_layers,
        ...
        params = ala2_chiro_cpainn_trainable_params_from_package(package)
        painn_state = package["state"]
        painn_config = package["config"]
        _write_painn_radial_cutoff_summary(run_dir, painn_config)
    elif model_type == "ala2_chiro_painn_jax":
        ...
        if painn_backend == "amboltz_aligned":
            package = init_painn_amboltz_aligned_velocity(
                init_key,
                problem,
                hidden_dim=hidden_dim,
                n_layers=n_layers,
    ...
    fm_optimizer = {
        "adam_beta1": 0.95 if model_type in {"painn", "ala2_cpainn", "ala2_chiro_cpainn", "ala2_chiro_painn_jax"} and problem.name == "ala2" else 0.9,
        "adam_beta2": 0.999,
        "adam_eps": 1.0e-8,
        "weight_decay": 0.01 if model_type == "painn" and problem.name == "ala2" else 0.0,
            ...
            config["model"].update(
                {
                    ...
                    "ala2_atom_identity": painn_config.get("ala2_atom_identity", "none"),
                    ...
                    "condition_name": painn_config.get("condition_name"),
                    "temp_min": painn_config.get("temp_min"),
                    "temp_max": painn_config.get("temp_max"),
                    "temp_embed_l0": painn_config.get("temp_embed_l0"),
                    "temp_embed_dim": painn_config.get("temp_embed_dim"),
                }
            )
            config["radial_cutoff_comparison"] = {
                "adtm_jax_painn": {
                    "radial_basis": painn_config.get("radial_basis"),
                    "cutoff": painn_config.get("cutoff"),
                    "radius": float(painn_config.get("radius", 0.0)),
                ...
                "amboltz_reference": {
                    "r_max": 0.8 if problem.name == "ala2" else None,
                    "num_radial_basis": 64 if problem.name == "ala2" else None,
                    "painn_py_cutoff": "polynomial",
                    "painn_aligned_py_cutoff": "cosine",
                },
            }
    ...
    def fm_package(current_params: Params) -> Any:
        if model_type == "ala2_cpainn":
            model_params, extra_params = split_ala2_cpainn_trainable_params(current_params)
            payload = {
                "model_type": "ala2_cpainn",
                "config": painn_config,
                "params": model_params,
                "state": painn_state,
            }
            ...
            return payload
        if model_type == "ala2_chiro_cpainn":
            model_params, extra_params = split_ala2_chiro_cpainn_trainable_params(current_params)
            payload = {
                "model_type": "ala2_chiro_cpainn",
                "config": painn_config,
                "params": model_params,
                "state": painn_state,
            }
        ...
        if model_type == "ala2_chiro_painn_jax":
            ...
            return payload
        if model_type == "painn":
            backend = painn_config.get("painn_backend", str(painn_backend)) if painn_config else str(painn_backend)
            payload = {
                "model_type": "painn",
                "painn_backend": backend,
                "config": painn_config,
                "params": current_params["model"] if backend != "amboltz_aligned" and isinstance(current_params, dict) and "model" in current_params else current_params,
                "state": painn_state,
            }
            if backend != "amboltz_aligned" and isinstance(current_params, dict) and "extra" in current_params:
    ...
    def quick_eval(current_params: Params, step: int) -> None:
        ...
        elif problem.name == "ala2":
            eval_chunk_size = min(256, n_eval_samples)
        else:
            eval_chunk_size = n_eval_samples
        eval_chunk_size = max(1, min(eval_chunk_size, n_eval_samples))

        if model_type == "ala2_cpainn":
            eval_params = {"model_type": "ala2_cpainn", "config": painn_config, "params": current_params, "state": painn_state}
        elif model_type == "ala2_chiro_cpainn":
            eval_params = {"model_type": "ala2_chiro_cpainn", "config": painn_config, "params": current_params, "state": painn_state}
        elif model_type == "ala2_chiro_painn_jax":
            eval_params = {"model_type": "ala2_chiro_painn_jax", "config": painn_config, "params": current_params, "state": painn_state}
        elif model_type == "painn":
            eval_params = {
                "model_type": "painn",
                "painn_backend": painn_config.get("painn_backend", str(painn_backend)) if painn_config else str(painn_backend),
                "config": painn_config,
        ...
        for j, beta in enumerate(eval_betas):
            ...
            try:
                if problem.name == "ala2":
                    ...
                    ref = load_ala2_positions_for_beta(beta, n=n_eval_samples, seed=960000 + int(step) + j)
                    item = ala2_summary_metrics(samples, ref, problem, energy_w2_samples=min(256, n_eval_samples), seed=970000 + int(step) + j)
                    print(
                        "[FM eval] "
                        f"step={int(step)} beta={float(beta):.8f} method={fm_eval_method} "
                        f"ew2={item.get('ew2', float('nan')):.6g} "
                        f"ew2_2k={item.get('ew2_2k', float('nan')):.6g} "
                        f"energy_mean={item.get('energy_mean', float('nan')):.6g} "
                        f"energy_std={item.get('energy_std', float('nan')):.6g} "
                        f"energy_finite={item.get('energy_finite_fraction', float('nan')):.4f} "
                        f"energy_q50={item.get('energy_finite_q50', float('nan')):.6g} "
                        f"energy_q90={item.get('energy_finite_q90', float('nan')):.6g} "
                        f"energy_q99={item.get('energy_finite_q99', float('nan')):.6g} "
                        f"md_energy_mean={item.get('md_energy_mean', float('nan')):.6g} "
                        f"min_pair_q01={item.get('min_pair_distance_q01', float('nan')):.6g} "
                        f"w2={item.get('w2', float('nan')):.6g} "
                        f"phi_w2={item.get('phi_w2', float('nan')):.6g} "
                        f"psi_w2={item.get('psi_w2', float('nan')):.6g}",
                        flush=True,
                    )
