...
"${PY}" scripts/audit_dw4_reference_v2.py \
  --bundle "${REFERENCE}" \
  --legacy-dir "${ROOT}/data/dw4_paper" \
  --output "${RUN}/reference_exact_identity_audit.json"
...
write_status plot_publication_figures
"${PY}" scripts/plot_dw4_reference_v2.py \
  --run-dir "${RUN}" \
  --reference "${REFERENCE}/eval/samples_beta_1.00.npy"

SCORE_METRICS="${RUN}/score_samples_metrics_dw4_reference_v2_eval_fm_am_betas_1p00_geo2000_ew20000.json"
...
sha256sum \
  ...
  "${RUN}/publication_figures/figure_metadata.json" \
  "${RUN}"/publication_figures/*.png \
  "${RUN}"/publication_figures/*.pdf \
